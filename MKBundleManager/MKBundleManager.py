...
class BundledAssemblyManagerView(Choose2):
	...
	def show(self):
		...
		if self.cmd_Item_SaveAs == None:
			self.cmd_Item_SaveAs = self.AddCommand("导出文件...", flags = idaapi.CHOOSER_POPUP_MENU, icon=139)
		
		if self.cmd_Item_ReplaceBy == None:
			self.cmd_Item_ReplaceBy = self.AddCommand("替换文件...", flags = idaapi.CHOOSER_POPUP_MENU, icon=139)
